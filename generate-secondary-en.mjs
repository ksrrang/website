...
const privacyKo = await readFile("privacy.html", "utf8");
const privacyCurrentEn = await readFile("privacy.en.html", "utf8");
let privacyEn = replaceArticles(privacyKo, privacyCurrentEn);
privacyEn = replaceAll(privacyEn, [
  ['<html lang="ko">', '<html lang="en">'],
  ["KSRAudio 개인정보처리방침입니다. 미디어 파일, 마이크, SMB·FTP·SFTP 접속정보, 저장 및 공유 기능의 데이터 처리 방식을 안내합니다.", "KSRAudio Privacy Policy covering media files, microphone use, SMB/FTP/SFTP credentials, storage, and sharing."],
  ["개인정보처리방침 | KSRAudio", "Privacy Policy | KSRAudio"],
  ["privacy.en.html?lang=en", "privacy.html?lang=ko"],
  [">홈으로<", ">Home<"],
  [">English<", ">한국어<"],
  ["<h1>개인정보처리방침</h1>", "<h1>Privacy Policy</h1>"],
  ["KSRRang(이하 “회사”)은 KSRAudio 이용자의 개인정보와 미디어 데이터를\n          중요하게 보호합니다. 이 방침은 KSRAudio가 어떤 정보에 접근하고, 해당\n          정보를 어떤 목적으로 처리하며, 어디에 저장하거나 전송하는지를 설명합니다.", "KSRRang (the “Company”) places great importance on protecting the personal information and media data of KSRAudio users. This policy explains what information KSRAudio accesses, why it processes that information, and where the information is stored or transmitted."],
  ...
  ["중요 안내", "Important"],
  ["KSRAudio는 이용자가 선택한 미디어 파일과 원격 서버의 콘텐츠를 회사 서버에\n          자동 업로드하지 않습니다. SMB·FTP·SFTP 사용 시 데이터는 이용자의 기기와\n          이용자가 직접 지정한 서버 사이에서 전송됩니다.", "KSRAudio does not automatically upload user-selected media or remote-server content to servers operated by the Company. With SMB, FTP, or SFTP, data is transferred between the user's device and the server the user specifies."],
...
const supportKo = await readFile("support.html", "utf8");
const supportCurrentEn = await readFile("support.en.html", "utf8");
let supportEn = replaceArticles(supportKo, supportCurrentEn);
supportEn = replaceAll(supportEn, [
  ['<html lang="ko">', '<html lang="en">'],
  ["KSRAudio 고객지원 페이지입니다. 재생, 편집, 보컬 제거, 오버더빙, SMB·FTP·SFTP 연결, 파일 저장과 공유 관련 도움말을 제공합니다.", "KSRAudio support for playback, editing, vocal removal, overdubbing, SMB/FTP/SFTP connections, file storage, and sharing."],
  ["KSRAudio 고객지원 페이지입니다. 재생, 편집, 보컬 제거, 오버더빙, SMB·FTP·SFTP 연결, 파일 저장과 공유 문제 해결 방법을 안내합니다.", "KSRAudio support for playback, editing, vocal removal, overdubbing, SMB/FTP/SFTP connections, file storage, and sharing."],
  ["고객지원 | KSRAudio", "Support | KSRAudio"],
  ['aria-label="지원 페이지 메뉴"', 'aria-label="Support navigation"'],
  ["support.en.html?lang=en", "support.html?lang=ko"],
  [">자주 묻는 질문<", ">FAQ<"],
  [">개인정보처리방침<", ">Privacy Policy<"],
  [">홈으로<", ">Home<"],
  [">English<", ">한국어<"],
  ["<h1>KSRAudio 고객지원</h1>", "<h1>KSRAudio Support</h1>"],
...
supportEn = supportEn
  ...
  .replace(
    '<a class="button" href="mailto:support@ksrrang.com?subject=KSRAudio%20Support">',
    '<a class="button button-primary" href="mailto:support@ksrrang.com?subject=KSRAudio%20Support">'
  )
  .replace(
...
for (const [name, html] of [["privacy.en.html", privacyEn], ["support.en.html", supportEn]]) {
